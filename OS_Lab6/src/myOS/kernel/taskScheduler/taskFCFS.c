...
//初始化就绪队列
void rqInitFCFS(myTCB* idleTask) {//对rq进行初始化处理
    rq.idleTask = idleTask;
}    //idleTask不入队

//如果就绪队列为空，返回True
int rqIsEmptyFCFS(void) {//当head和tail均为NULL时，rq为空
...
void taskEnqueueRqFCFS(myTCB* task) {//将task入队rq
    if (rqIsEmptyFCFS()) {
        rq.head = task;
        rq.tail = task;
    }
...
//获取就绪队列的头结点信息，并返回
myTCB* rqNextTaskFCFS(void) {//获取下一个Task
    if (!rq.head) {
        return &tcbPool[1];
    }
    else {
...
//如果等待队列为空，返回True
int wqIsEmptyFCFS(void) {//当head和tail均为NULL时，wq为空
...
void taskEnqueueWqFCFS(myTCB* task) {//将task入队wq
    if (wqIsEmptyFCFS()) {
        wq.head = task;
        wq.tail = task;
    }
...
//将等待队列中的TCB移除
void taskDequeueWqFCFS(myTCB* task) {//wq出队
    if (wq.head == wq.tail) {
        if (task != wq.head) {
            return;
        }
...
//获取等待队列的头结点信息，并返回
myTCB* wqNextTaskFCFS(void) {//获取下一个Task
    if (!wq.head) {
        return &tcbPool[1];
    }
    else {
...
void setSchedulerFCFS(scheduler* sch) {
    scheduler schedulerFCFS = {
        .type = FCFS,
        .preemptiveOrNot = 0,
        .nextTaskFunc = rqNextTaskFCFS,
        .enqueueTaskFunc = taskEnqueueRqFCFS,
        .dequeueTaskFunc = taskDequeueRqFCFS,
        .schedulerInitFunc = rqInitFCFS,
